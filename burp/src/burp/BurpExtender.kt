...
class BurpExtender : IBurpExtender, IProxyListener, IExtensionStateListener {
    ...
    override fun processProxyMessage(messageIsRequest: Boolean, message: IInterceptedProxyMessage?) {
        ...
        if (messageInfo != null) {
            ...
            // Rebuild the request with both PwnFox and clean headers
            val newHeaders = ArrayList<String>()
            newHeaders.addAll(cleanHeaders)
            newHeaders.addAll(pwnFoxHeaders)  // Add PwnFox headers back
            messageInfo.request = helpers.buildHttpMessage(newHeaders, body)
        }
    }
}